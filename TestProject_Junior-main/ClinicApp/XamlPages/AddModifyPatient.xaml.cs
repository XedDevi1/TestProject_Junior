...
namespace ClinicApp.XamlPages
{
    ...
    public partial class AddModifyPatient : Page
    {
        ...
        {
            datePickerBirth.DisplayDateStart = DateTime.Now.AddYears(-150);
        ...
        private async void btnSubmit_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                {
                    status = false;
                    messageBuilder.Append("Дата рождения не может быть в будущем времени\n");
                }
                if (dateOfBirth < DateTime.Now.AddYears(-150))
                {
                    status = false;
                    messageBuilder.Append("Дата рождения слишком поздняя\n");
                }
                #endregion
